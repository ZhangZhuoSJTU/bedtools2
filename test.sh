#!/bin/bash
# Compile, then run all ProgramBench test branches for arq5x__bedtools2.dd57059.
#
...
INSTANCE=arq5x__bedtools2.dd57059
...
if [ -n "$cmd" ]; then exec /usr/bin/script $opts "$file" /bin/sh -c "$cmd"; else exec /usr/bin/script $opts "$file"; fi
SH
chmod +x "$SHIM/script"
...
while read -r branch; do
    ...
    # bedtools' BAM tests drive I/O through an htsutil helper; the tarball ships
    # prebuilt Linux x86-64 ELF copies of it (in test/ and eval/tests/resources/)
    # that give "Exec format error" on macOS. Overlay every copy with the native
    # htsutil compile.sh just built alongside bedtools.
    if [ -f test/htsutil ]; then
        find "$dir" -name htsutil -type f -exec cp test/htsutil {} \; -exec chmod +x {} \;
    fi
